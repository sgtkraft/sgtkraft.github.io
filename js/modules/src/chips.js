(function ($) {
  ...
  class MaterialChip {
    ...
    handleSelecorChips() {
      ...
      this.$document.on('click', this.selectors.chips, e => $(e.target).find(this.selectors.input).focus());
    }

    handleBlurInput() {

      this.$document.on('blur', this.selectors.chips, () => setTimeout(() => this.ulWrapper.hide(), 100));
    }

    handleSelectorChip() {

      this.chips.on('click', '.chip', e => $(e.target).removeClass('selected').addClass('selected'));
    }
    ...
    handleDocumentFocusOut() {
      ...
      this.$document.on('focusout', `${this.selectors.chips} ${this.selectors.input}`, e => $(e.target).closest(this.selectors.chips).removeClass('focus'));
    }
    ...
    inputKeyDown() {
      ...
      const $thisChups = this.chips;
      const $input = $thisChups.children('.chip-position-wrapper').children('input');
        ...
        if ($inputValue.length) {
          ...
          for (const item in dataChip) {
            ...
            if (dataChip[item].toLowerCase().includes($inputValue.toLowerCase())) {
              ...
              $thisChups.children('.chip-position-wrapper').append($ulWrapper.append($(`<li>${dataChip[item]}</li>`)));
            }
          }
        }
