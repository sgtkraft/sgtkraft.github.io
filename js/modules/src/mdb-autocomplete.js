    ...
    changeSVGcolors() {
      ...
      if (this.$input.hasClass('mdb-autocomplete')) {
        ...
        this.$input.on('click keyup', e => {
          ...
          e.preventDefault();
          $(e.target).parent().find('.mdb-autocomplete-clear').find('svg').css('fill', this.options.xColor);
        });

        this.$input.on('blur', e => {

          e.preventDefault();
          $(e.target).parent().find('.mdb-autocomplete-clear').find('svg').css('fill', this.options.xBlurColor);
        });
      }
    }
  }
